...
public sealed class ZoneValidator : AbstractValidator<Zone>
{
    ...
    public ZoneValidator()
    {
        ...
        RuleFor(x => x.CurrentStreamId)
            ...
            .When(x => !string.IsNullOrEmpty(x.CurrentStreamId));
        ...
        RuleFor(x => x.DefaultVolume)
            .InclusiveBetween(0, 100)
            .WithMessage("Default volume must be between 0 and 100.")
            .Must((zone, defaultVolume) => defaultVolume >= zone.MinVolume && defaultVolume <= zone.MaxVolume)
            .WithMessage("Default volume must be between minimum and maximum volume.");

        RuleFor(x => x.MinVolume).InclusiveBetween(0, 100).WithMessage("Minimum volume must be between 0 and 100.");

        RuleFor(x => x.MaxVolume).InclusiveBetween(0, 100).WithMessage("Maximum volume must be between 0 and 100.");
