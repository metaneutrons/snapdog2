...
public class AudioStreamConfiguration : IEntityTypeConfiguration<AudioStream>
{
    ...
    public void Configure(EntityTypeBuilder<AudioStream> builder)
    {
        ...
        builder.Property(static x => x.BitrateKbps).IsRequired();
        ...
        builder.HasIndex(static x => x.Name).HasDatabaseName("IX_AudioStreams_Name");

        builder.HasIndex(static x => x.Status).HasDatabaseName("IX_AudioStreams_Status");

        builder.HasIndex(static x => x.Codec).HasDatabaseName("IX_AudioStreams_Codec");

        builder.HasIndex(static x => x.CreatedAt).HasDatabaseName("IX_AudioStreams_CreatedAt");
    }
}
