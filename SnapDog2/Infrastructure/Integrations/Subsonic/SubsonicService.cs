...
public partial class SubsonicService : ISubsonicService, IAsyncDisposable
{
    ...
    [LoggerMessage(2900, LogLevel.Information, "🚀 Initializing Subsonic connection to {Url}")]
    private static partial void LogInitializing(ILogger logger, string url);

    [LoggerMessage(
        2901,
        LogLevel.Information,
        "🚀 Attempting Subsonic connection to {Url} (attempt {AttemptNumber}/{MaxAttempts}: {ErrorMessage})"
    )]
    ...
    [LoggerMessage(2902, LogLevel.Information, "Subsonic connection established successfully to {Url}")]
    private static partial void LogConnectionEstablished(ILogger logger, string url);

    [LoggerMessage(2903, LogLevel.Information, "Subsonic service initialized for server: {Url}, user: {Username}")]
    private static partial void LogSubsonicServiceInitialized(ILogger logger, string url, string username);

    [LoggerMessage(2904, LogLevel.Error, "Subsonic service initialization failed: {ErrorMessage}")]
    private static partial void LogInitializationFailed(ILogger logger, string errorMessage);

    [LoggerMessage(2905, LogLevel.Warning, "Subsonic service is disabled in configuration")]
    private static partial void LogSubsonicDisabled(ILogger logger);

    [LoggerMessage(2906, LogLevel.Debug, "Getting playlists from Subsonic server")]
    private static partial void LogGettingPlaylists(ILogger logger);

    [LoggerMessage(2907, LogLevel.Information, "Retrieved {Count} playlists from Subsonic server")]
    ...
    [LoggerMessage(2914, LogLevel.Debug, "Retrieved stream URL for track: {TrackId}")]
    private static partial void LogStreamUrlRetrieved(ILogger logger, string trackId);

    [LoggerMessage(2915, LogLevel.Warning, "Stream URL not found for track: {TrackId}")]
    private static partial void LogStreamUrlNotFound(ILogger logger, string trackId);

    [LoggerMessage(2916, LogLevel.Error, "Failed to get stream URL for track: {TrackId}")]
    private static partial void LogGetStreamUrlError(ILogger logger, string trackId, Exception ex);

    [LoggerMessage(2917, LogLevel.Debug, "Testing connection to Subsonic server")]
    private static partial void LogTestingConnection(ILogger logger);

    [LoggerMessage(2918, LogLevel.Information, "Subsonic connection test successful")]
    private static partial void LogConnectionTestSuccessful(ILogger logger);

    [LoggerMessage(2919, LogLevel.Warning, "Subsonic connection test failed")]
    private static partial void LogConnectionTestFailed(ILogger logger);

    [LoggerMessage(2920, LogLevel.Error, "Subsonic connection test error")]
    private static partial void LogConnectionTestError(ILogger logger, Exception ex);

    [LoggerMessage(2921, LogLevel.Information, "Subsonic service disposed")]
    private static partial void LogSubsonicServiceDisposed(ILogger logger);

    [LoggerMessage(2922, LogLevel.Warning, "Failed to publish notification {NotificationType}")]
