...
public partial class EnhancedPerformanceCommandBehavior<TCommand, TResponse>(
    ILogger<EnhancedPerformanceCommandBehavior<TCommand, TResponse>> logger,
    EnterpriseMetricsService metricsService
) : ICommandPipelineBehavior<TCommand, TResponse>
    where TCommand : ICommand<TResponse>
    where TResponse : IResult
{
    private readonly ILogger<EnhancedPerformanceCommandBehavior<TCommand, TResponse>> _logger = logger;
    ...
    private void RecordCommandSpecificMetrics(TCommand command, TResponse response, double durationSeconds)
    {
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogDebug(ex, "Failed to record command-specific metrics for {CommandName}", typeof(TCommand).Name);
        }
    }
