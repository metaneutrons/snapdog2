    ...
    : ICommandHandler<ZoneNameCommand, Result>
{
    ...
    [LoggerMessage(EventId = 114900, Level = LogLevel.Information, Message = "Handling zone name command for zone {ZoneIndex} with name '{Name}'")]
    private partial void LogHandling(int zoneIndex, string name);

    [LoggerMessage(EventId = 114901, Level = LogLevel.Warning, Message = "Zone {ZoneIndex} not found")]
    private partial void LogZoneNotFound(int zoneIndex);

    [LoggerMessage(EventId = 114902, Level = LogLevel.Warning, Message = "Zone name setting not implemented for zone {ZoneIndex}")]
