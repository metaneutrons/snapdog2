...
) : IQueryHandler<CommandStatusQuery, Result<string>>
{
    ...
    [LoggerMessage(EventId = 113200, Level = LogLevel.Debug, Message = "Retrieving command processing status"
)]
    private partial void LogRetrievingCommandStatus();

    [LoggerMessage(EventId = 113201, Level = LogLevel.Warning, Message = "Failed → retrieve command status: {ErrorMessage}"
)]
