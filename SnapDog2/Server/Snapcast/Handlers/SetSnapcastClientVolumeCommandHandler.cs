...
) : ICommandHandler<SetSnapcastClientVolumeCommand, Result>
{
    ...
    [LoggerMessage(EventId = 114400, Level = LogLevel.Information, Message = "Setting volume for Snapcast client {ClientIndex} → {Volume}% (Source: {Source})"
)]
    private partial void LogSettingClientVolume(string clientIndex, int volume, string source);

    [LoggerMessage(EventId = 114401, Level = LogLevel.Error, Message = "Failed → set volume for Snapcast client {ClientIndex}"
)]
